...
#		self.f=docx.Document("ID.docx")
	...
	def getinfo16(self):	#认购确认(120)，预约认购确认(121)
		self.gettableinfo("st120.txt")
	...
	def getinfo62(self):	#基金质押确认（188）
		self.gettableinfo("st188.txt")
	def getinfo63(self):	#ETF申购申请（091）
		self.gettableinfo("st091.txt")
	def getinfo64(self):	#ETF赎回申请（093）
		self.gettableinfo("st093.txt")
	def getinfo65(self):	#ETF申购一次确认（191）
	...
	def getinfo68(self):	#ETF赎回二次确认（194）
		self.gettableinfo("st194.txt")
